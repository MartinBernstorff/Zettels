...
// Highlight with jQuery from:
// https://stackoverflow.com/questions/41533785/how-to-highlight-search-text-in-h
// tml-with-the-help-of-js @todo: This is the only use of jQuery. Find a vanila
// JS way
    ...
    function innerHighlight(node, pat) {
        ...
        if (node.nodeType == 3) {
            ...
            if (pos >= 0) {
                ...
                middlebit
                    .parentNode
                    .replaceChild(spannode, middlebit);
...
function loadIndex() {
    fetchJSON(function (response) {
        ...
        notes
            ...
            .forEach(note => {
                ...
                if (note.thumbnail === "") {
                    ...
                    // @todo: this loads the first image of the note for every note in the vault
                    // this isn't efficient at all 2 ideas: find a way of resizing with hugo use
                    // lazy loading.
        ...
        // @todo: wip notes.tags.forEach(tag => { 	const child =
        // document.createElement("li"); 	child.innerHTML = '<a
        // onclick="focusTag(this)">' + tag + '</a>' 	tags.append(child) });
        //
    });
}
...
function fetchJSON(callback) {
    ...
    const requestURL = '/test_garage_door/index.json';
...
// Prefetching https://github.com/turbolinks/turbolinks/issues/313
const hoverTime = 300
const fetchers = {}
const doc = document
    .implementation
    .createHTMLDocument('prefetch')
...
function prefetchTurbolink(url) {
    fetchPage(url, responseText => {
        ...
        doc.close()
        const snapshot = Turbolinks
            .Snapshot
            .fromHTMLElement(doc.documentElement)
        Turbolinks
            .controller
            .cache
            .put(url, snapshot)
...
function prefetched(url) {
    return location.href === url || Turbolinks
        .controller
        .cache
        .has(url)
}
...
document.addEventListener('mouseover', event => {
    if (!event.target.dataset.prefetch) 
        return
    ...
    event
        .target
        .addEventListener('mouseleave', cleanup)
